...
def calc_stats(rx,ry,gdf,ndist,trsfm,tps):
        ...
        # matrices vides aux dimensions du "bloc temporelle" (len(tps) correspond à l'axe 0 de la mat tps) et du nombre de districts/aires/pays (ndist)
...
def calc_moy(ddirout,deb,fin,pays,niveau,types,sat,prod,res_temp,res,varname,shape):
    ...
    andeb = datetime.strptime(deb,"%Y-%m-%d").strftime("%Y")
    mdeb = datetime.strptime(deb,"%Y-%m-%d").strftime("%B")[0:3]
    datefin = datetime.strptime(fin,"%Y-%m-%d")
    anfin = datetime.strptime(fin,"%Y-%m-%d").strftime("%Y")
    mfin = datetime.strptime(fin,"%Y-%m-%d").strftime("%B")[0:3]
    ...
    if varname == "Deep_Blue_Aerosol_Optical_Depth_550_Land":
        ...
        varname1 = varname
    tempo_dict = {"d":"daily","w":"weekly","m":"monthly","t":"trimester"}
    output = varname1+'_'+files[0][:-5]+'_'+niveau+'_'+shape+'_'+pays+'_'+andeb+mdeb+"_"+anfin+mfin+'_'+tempo_dict[res_temp]+'.nc'
